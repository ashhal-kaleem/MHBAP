...
async def get_user_analytics(db: AsyncSession, user_id: uuid.UUID) -> UserAnalytics:
    ...
    # 3. Emotion counts per session via SQL GROUP BY (avoids loading all raw string rows into RAM)
    label_counts_result = await db.execute(
        ...
        .where(Prediction.session_id.in_(session_ids))
        .group_by(Prediction.session_id, Prediction.emotion_label)
    )
    counts_by_sid: dict = defaultdict(dict)
    global_counts: Counter = Counter()
    total_labels_count = 0
    for row in label_counts_result.all():
        counts_by_sid[row.session_id][row.emotion_label] = row.cnt
        global_counts[row.emotion_label] += row.cnt
        total_labels_count += row.cnt
    ...
    for s in sessions:
        ...
        sess_emotions = counts_by_sid.get(s.id, {})
        dominant = max(sess_emotions, key=sess_emotions.get) if sess_emotions else None
    ...
    return UserAnalytics(
        ...
        emotion_breakdown=EmotionBreakdown(counts=dict(global_counts), total=total_labels_count),
    )
