...
# ─── 1. Stub ml.SessionRunner BEFORE any backend imports ───────────────────
...
fake_sr_mod = types.ModuleType("ml.SessionRunner")
fake_sr_mod.SessionRunner = MockSessionRunner  # type: ignore[attr-defined]
sys.modules["ml.SessionRunner"] = fake_sr_mod
...
def _make_app(
    ...
    patcher = patch("app.services.SessionService.get_session", new_callable=AsyncMock, return_value=mock_session)
    patcher.start()
