...
class TestPasswordStrength:
    @pytest.fixture(autouse=True)
    def patch_redis(self):
        with patch("app.core.RateLimit.get_redis", return_value=_fake_redis):
            yield
...
class TestResourceOwnership:
    ...
    def test_analytics_owner_access(self):
        ...
        with patch("app.services.AnalyticsService.get_user_analytics", new=AsyncMock(return_value={"test": "data"})):
            with TestClient(self._make_app(), raise_server_exceptions=False) as c:
